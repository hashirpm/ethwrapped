...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json({
    ...
    totalEthRecieved: (txnProcessedData.totalEthReceived / 1e18).toFixed(4),
    totalEthSent: (txnProcessedData.totalEthSent / 1e18).toFixed(4),
    cumulativeGasUsed: (txnProcessedData.cumulativeGasUsed / 1e18).toFixed(4),
